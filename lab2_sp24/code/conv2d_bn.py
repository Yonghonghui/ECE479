...
def conv2d_bn(x,
    ...
    # Your code goes here
#     Conv2D_name = generate_layer_name('Conv2D', prefix=name)
    x = Conv2D(filters=filters,kernel_size=kernel_size,strides=(strides,strides),padding=padding,use_bias=use_bias,name=name)(x)
    ...
    if not use_bias:
        ...
        # Your code goes here
        x=BatchNormalization(axis=bn_axis,momentum=0.995,epsilon=0.001,scale=False)(x)
